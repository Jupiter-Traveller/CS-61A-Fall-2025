...
def boar_brawl(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    score = (opponent_score // 10 % 10) - (player_score % 10)
    score = abs(score)
    if score == 0:
        return 1
    else:
...
def is_always_roll(strategy, goal=GOAL):
    ...
    sign = strategy(my_score, opponent_score)
    while my_score < goal:
        opponent_score = 0
        while opponent_score < goal:
            if strategy(my_score, opponent_score) != sign:
                return False
            opponent_score += 1
        my_score += 1
    return True
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    i, max_score, sign = 1, 0, 1
    while i <= 10:
        mid = make_averaged(roll_dice, times_called)(i, dice)
        if mid > max_score:
            sign = i
            max_score = mid
        i += 1
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    if sus_update(0, score, opponent_score) >= GOAL:
        ...
        return sus_strategy(score, opponent_score, threshold)
