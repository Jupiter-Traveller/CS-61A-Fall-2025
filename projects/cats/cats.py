...
def accuracy(typed: str, source: str) -> float:
    ...
    length = 0
    if len_source == 0 and len_typed == 0:
        return 100.0
    if  len_typed != 0 and len_source != 0:
        min_len = min(len_source, len_typed)
        for i in range (min_len):
            if typed_words[i] == source_words[i]:
                length += 1
        return length / len_typed * 100
    return 0.0
    # END PRaOBLEM 3
...
def autocorrect(typed_word: str, word_list: list[str], diff_function, limit: int) -> str:
    ...
    result_list = [fun(word) for word in word_list if fun(word) <= limit]
...
def furry_fixes(typed: str, source: str, limit: int) -> int:
    ...
    """
    # BEGIN PROBLEM 6
    def fun(word, standard, current):
        if current > limit:
            return 0
        if word == standard:
            return 0
        if len(word) == 0 or len(standard) == 0:
            return 0
        if word[0] == standard[0]:
            return fun(word[1:], standard[1:], current)
        else:
            return 1 + fun(word[1:], standard[1:], current + 1)
    return fun(typed, source, 0) + abs(len(typed) - len(source))
    # END PROBLEM 6
...
def report_progress(typed: list[str], source: list[str], user_id: int, upload) -> float:
    ...
    "*** YOUR CODE HERE ***"
    def fun(input, stan):
        if len(input) == 0 or len(stan) == 0:
            return 0
        if input[0] != stan[0]:
            return 0
        else:
            return 1 + fun(input[1:], stan[1:])
    ratio = fun(typed[:], source[:]) / len(source)
...
def fastest_words(words_and_times: dict) -> list[list[str]]:
    ...
    for i in word_indices:
        compare = [get_time(times, j, i) for j in player_indices]
        min_time = min(compare)
