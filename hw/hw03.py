...
def digit_distance(n):
    ...
    """
    "*** YOUR CODE HERE ***"
    last, before_last, all_but_last = n % 10, (n // 10) % 10, n // 10
    if n < 10:
        return 0
    else:
        return abs(before_last - last) + digit_distance(all_but_last)
...
def interleaved_sum(n, f_odd, f_even):
    ...
    def add_odd(k):
        if k <= n:
            return f_odd(k) + add_even(k + 1)
        else:
            return 0
    def add_even(k):
        if k <= n:
            return f_even(k) + add_odd(k + 1)
        else:
            return 0
    return add_odd(1)
    # another solution
    ...
    # def add_odd(k):
    #     if k <= n:
    #         return f_odd(k) + add_odd(k + 2)
    #     else:
    #         return 0
    # def add_even(k):
    #     if k <= n:
    #         return f_even(k) + add_even(k + 2)
    #     else:
    #         return 0
    # return add_odd(1) + add_even(2)
...
def count_dollars(total):
    ...
    "*** YOUR CODE HERE ***"
    small = 100
    def track(total, small):
        if total // small != 0:
            return small
        else:
            return track(total, next_smaller_dollar(small))
    next_small = track(total, small)
    
    def cal(total, next_small):
        if next_small == 1:
            return 1
        elif total < 0:
            return 0
        else:
            return cal(total, next_smaller_dollar(next_small)) + cal(total-next_small, next_small)
    return cal(total, next_small)
